# updated map-reduce example with parallel execution of tools using ThreadPoolExecutor
...
def agent_node(state: State) -> State:
    ...
    if hasattr(resp, "tool_calls") and resp.tool_calls:
        ...
        # Execute tool calls in parallel while preserving ordering of results
        max_workers = min(3, len(tool_calls)) if tool_calls else 1
        tool_results: List[Any] = []
        with ThreadPoolExecutor(max_workers=max_workers) as ex:
            futures = [ex.submit(tool_executor, tc) for tc in tool_calls]

            # Collect results in submission order to preserve ordering
            for fut in futures:
                try:
                    res = fut.result()
                except Exception as e:
                    # Capture exceptions as strings so downstream code can inspect
                    res = f"tool_error: {e}"
                tool_results.append(str(res))  # Normalize to string for aggregation
    else:
        tool_results = None
...
if __name__ == "__main__":
    query = "what is 5 times 80 and 18th of 90 and also 20 times half?"
